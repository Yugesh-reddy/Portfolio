...
3. **AdaTTT: Test-Time Adaptation That Did Not Pay, and the Fallback That Did** (Mar 2026 - Jul 2026)
   ...
   - Question: under real distribution shift, does test-time adaptation recover enough accuracy to justify its compute, and if not, what does?
   - Encoder result: swapping frozen ViT-B/16 + BERT for CLIP ViT-B/16 with the fusion stack unchanged moved VQA-v2 val from 59.93 to 67.50 official soft accuracy, a controlled +7.57 point encoder effect.
   - Debugging: the first run was silently broken. The gate's auxiliary loss outweighed the answer loss 19x on the fusion gradient, <UNK> was a positive training target on 31% of questions, and the logged metric credited those <UNK> answers. All three were found from gradients, optimizer state and labels, and fixed with regression tests.
   - Main finding: gaussian noise costs 6.40 points. Adaptation was tested at the original step size, at a step size chosen on held-out data, and behind a gate trained to predict per-sample benefit. Best result +0.18 points, 95% CI -0.11 to +0.47. A per-sample oracle shows +1.72 is available, but no logged signal predicts which samples benefit above AUROC 0.57, so the limit is the benefit signal, not the step size.
   - Protocol: selection and stop rules were fixed in code before each run, the gate specification had to be committed to git before the scorer would read the evaluation split, FLOPs counted deployment cost including the backward pass through every augmented view, and results reproduced exactly across sessions.
   - What worked instead: confidence-based abstention. One threshold fit on a held-out split raises accuracy on answered questions by 5.3 points at 90% coverage and 10.4 at 80%, holds across clean, blurred and noised images, and costs no extra compute. Under heavy noise, answering the confident 80% beats the clean model answering everything.
   - Engineering: preemption-safe Colab A100 orchestration (token refresh, orphan-VM recovery, verified checkpoint relay, hard budget cap) tested across 18 simulated failure scenarios, 280 tests in CI, an answer-or-abstain Gradio demo, and a published release whose artifacts rebuild every number and figure byte-for-byte on a CPU.
   - Context: began as CS 518 (Deep Learning for Computer Vision, UIC) with Aishwarya Reddy Chinthalapudi and Aryan Shetty; the follow-up study is the solo continuation.
