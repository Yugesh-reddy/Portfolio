...
The site displays your track, artist, album, cover artwork and track duration. During playback it also shows the position returned by Spotify, refreshed with the visible tile every 30 seconds; it does not estimate progress between polls. It never controls playback or asks visitors to sign in. Until connected, the tile says “Spotify is not connected yet.” When nothing is playing, the tile shows what you last played, resolved in this order: the paused track itself (Spotify only writes history once a track finishes or is skipped, so a paused track may never appear there), then the last track this server saw playing (covers quitting the app mid-track; private sessions are never remembered), then your Spotify history. With none of those, the tile shows an idle state. Upstream failures show an unavailable state rather than an invented song.

The widget uses the site's monochrome UI with unaltered album artwork. Album and duration details are optional; older responses still render. The equalizer and progress indicator appear only for a playing track. A recent track shows its duration and a Spotify link, without suggesting playback controls.
